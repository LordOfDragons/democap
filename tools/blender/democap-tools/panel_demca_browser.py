...
class LIST_UL_DemcaBrowser(bpy.types.UIList):
    ...
    def draw_item(self, context, layout, data, item, icon,
        ...
        if self.layout_type in {'DEFAULT', 'COMPACT'}:
            ...
            if item.isDirectory:
                icon = 'TRIA_LEFT' if item.name == '..' else 'TRIA_RIGHT'
                layout.operator("democaptools_demcabrowser.changedirectory",
                                text="", icon=icon).path = item.path
...
class DemcaBrowserSelectionInfo(bpy.types.PropertyGroup):
    """Group of properties representing information about selected list item."""
    ...
    playtime: bpy.props.FloatProperty(
        "Playtime", default=0, precision=1, soft_min=0, min=0, soft_max=10)
    frameRate: bpy.props.IntProperty(
        "Framerate", default=0, soft_min=15, min=0, soft_max=90)
    pathAnimation: bpy.props.StringProperty(name="Path Animation", default="")
    pathDevicesRig: bpy.props.StringProperty(
        name="Path Devices Rig", default="")
    pathDevicesAnimation: bpy.props.StringProperty(
        name="Path Devices Animation", default="")
    pathObjectRig: bpy.props.StringProperty(name="Path Object Rig", default="")
    pathObjectAnimation: bpy.props.StringProperty(
        name="Path Object Animation", default="")
...
def DemcaBrowser_SelectionChanged(self, context):
    ...
    if index >= 0 and index < len(dtlist):
        ...
        if not entry.isDirectory:
            ...
            if demca.success:
                bsi = dtprops.browserSelectionInfo
                bsi.filename = entry.name
                bsi.timestamp = demca.formattedTimestamp
                bsi.character = demca.characterProfile
                bsi.configuration = demca.characterConfiguration
                bsi.playtime = demca.playtime
                bsi.frameRate = demca.frameRate
                bsi.pathAnimation = demca.pathAnimation
                bsi.pathDevicesRig = demca.pathDevicesRig
                bsi.pathDevicesAnimation = demca.pathDevicesAnimation
                bsi.pathObjectRig = demca.pathObjectRig
                bsi.pathObjectAnimation = demca.pathObjectAnimation
                return
    ...
    bsi.pathAnimation = ""
    bsi.pathDevicesRig = ""
    bsi.pathDevicesAnimation = ""
    bsi.pathObjectRig = ""
    bsi.pathObjectAnimation = ""
...
class DemocapToolsPanelProperties(bpy.types.PropertyGroup):
    ...
    browserSelectionInfo: bpy.props.PointerProperty(
        type=DemcaBrowserSelectionInfo)
...
class LIST_OT_DemcaBrowserChdir(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        dtlist = wm.democaptools_filelistdemca
        if self.path:
            wm.democaptools_properties.currentDirectory = self.path
...
class VIEW3D_PT_DemocapToolsDemcaBrowser(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row.operator("democaptools_demcabrowser.changedirectory",
                     text="Captured", icon='FILE_FOLDER').path = \
            config.pathCaptureAnimations
        row.operator("democaptools_demcabrowser.changedirectory",
                     text="Projects", icon='FILE_FOLDER').path = \
            config.pathProjects

        layout.label(text="Directory: {}".format(
            os.path.split(dtprops.currentDirectory)[1]))

        row = layout.row()
        row.template_list("LIST_UL_DemcaBrowser", "File List", windowManager,
                          "democaptools_filelistdemca", windowManager,
                          "democaptools_filelistdemca_index")
        ...
        # import
        column = layout.row().column(align=True)
        column.operator("democaptools_demcabrowser.importanimation",
                        icon='ACTION')
        column.operator("democaptools_demcabrowser.importdevicesrig",
                        icon='ARMATURE_DATA')
        column.operator("democaptools_demcabrowser.importdevicesanimation",
                        icon='ACTION')
        column.operator("democaptools_demcabrowser.importobjectrig",
                        icon='ARMATURE_DATA')
        column.operator("democaptools_demcabrowser.importobjectanimation",
                        icon='ACTION')
...
def panelDemcaBrowserRegister():
    ...
    bpy.types.WindowManager.democaptools_properties = \
        bpy.props.PointerProperty(type=DemocapToolsPanelProperties)
    bpy.types.WindowManager.democaptools_filelistdemca = \
        bpy.props.CollectionProperty(type=DemcaBrowserItem)
    bpy.types.WindowManager.democaptools_filelistdemca_index = \
        bpy.props.IntProperty(name="Index for filelistdemca",
                              default=0,
                              update=DemcaBrowser_SelectionChanged)
