...
class VIEW3D_PT_DemocapToolsLiveConnect(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if (liveConnection is not None and liveConnection.connection_state ==
                dnl.Connection.ConnectionState.DISCONNECTED):
            liveConnection.dispose()
            liveConnection = None
            params.connection_status = "Disconnected"
...
class VIEW3D_PT_DemocapToolsLiveRecord(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row.column(align=True).prop(params,
                                    "sync_recording", expand=True, toggle=True)
...
def onTimerRecord():
    params = bpy.context.window_manager.democaptoolslive_params
    screen = bpy.context.screen
    if (not params.record or screen.is_scrubbing
            or not screen.is_animation_playing):
        params.record = False
        bpy.context.view_layer.update()
        return None
    return 0.01
...
class DemocapLiveParameters(bpy.types.PropertyGroup):
    connect_host: bpy.props.StringProperty(
        name="Host",
        description="Hostname or IP to connect to",
        default="localhost")

    connect_port: bpy.props.IntProperty(
        name="Port",
    ...
    record: bpy.props.BoolProperty(
        ...
        update=updateRecord,
        options={'SKIP_SAVE'})
    sync_recording: bpy.props.BoolProperty(
        name="Sync",
...
def onPostLoad(always_none):
    if liveConnection is not None:
        params = bpy.context.window_manager.democaptoolslive_params
        params.connection_status = liveConnection.infoStatus
